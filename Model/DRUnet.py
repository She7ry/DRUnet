...
from module.MDRM import MDRM
from module.MLDA import MLDA
...
class Up(nn.Module): # 定义一个名为 Up 的类，继承自 nn.Module
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = MDRM(in_channels, out_channels)
        else:
            # 使用转置卷积进行上采样
...
class UNet(nn.Module): # 定义一个名为 UNet 的类，继承自 nn.Module
    def __init__(self,
        ...
        # 定义 U-Net 的各个组件
        self.in_conv = MDRM(in_channels, base_c)              # 输入通道数: in_channels -> base_c (64)
        ...
        self.mlda1 = MLDA(base_c * 8)
        self.mlda2 = MLDA(base_c * 4)
        self.mlda3 = MLDA(base_c * 2)
    # 定义前向传播函数
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        skip3 = self.mlda2(x3)
        skip2 = self.mlda3(x2)
        x = self.up1(x5, skip4)       # 输入尺寸: (N, base_c*16//factor, H/8, W/8)，输出尺寸: (N, base_c*8//factor, H/8, W/8)
        x = self.up2(x, skip3)        # 输入尺寸: (N, base_c*8//factor, H/4, W/4)，输出尺寸: (N, base_c*4//factor, H/4, W/4)
        x = self.up3(x, skip2)        # 输入尺寸: (N, base_c*4//factor, H/2, W/2)，输出尺寸: (N, base_c*2//factor, H/2, W/2)
